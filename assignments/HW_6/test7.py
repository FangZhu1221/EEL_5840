# -*- coding: utf-8 -*-
"""
Created on Wed Nov  8 09:48:13 2017
...
for j in range(1000):
    for i in range(length):
        ...
        w1 = w1 + ((step1*np.asscalar(der_sig(t3))*e)*x1.T@np.multiply(der_sig(t1).T,(w2[0,0:2]*der_sig(t2[0])*w3[0,0]+w2[1,0:2]*der_sig(t2[1])*w3[0,1]))).T 
        #w1 = w1 + (np.matrix(np.asscalar(step1*e*der_sig(t3))*np.multiply(der_sig(t1),(np.matrix(w2[:,0:2]).T@np.multiply(der_sig(t2).T,np.matrix(w3[0,0:2]).T))))@x1)
        if j == 999:
            print(y)
