# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 12:53:17 2017
...
length = 16
training = np.zeros([16,2])
validate = np.zeros([1,16])
for j in range(4):
    if j == 3 or j == 0:
        k1 = 1
    else: 
    ...
    if j == 1 or j == 0:
        ...
        k2 = -1
    for i in range(4):
        training[j*4+i,0]=k1*np.random.uniform(0,2)
        training[j*4+i,1]=k2*np.random.uniform(0,2)
        if k1*k2>0:
            validate[0,j*4+i]=0
        else:
            if k1*k2<0:
...
p1.scatter(training[0:4,0],training[0:4,1],c="b")
p1.scatter(training[4:8,0],training[4:8,1],c="r")
p1.scatter(training[8:12,0],training[8:12,1],c="b")
p1.scatter(training[12:16,0],training[12:16,1],c="r")
p1.plot(t,z1)
p1.plot(t,z2)
