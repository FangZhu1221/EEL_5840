# -*- coding: utf-8 -*-
"""
Created on Thu Nov 30 09:47:53 2017
...
def MLP_Learning(x,l,activation,step,dropout):
    ...
    if activation == 0:
        ...
        if activation == 1:
            hd_output = ReLu(net1)
    if dropout == 1:
        hd = np.multiply(hd_output,P)
    else:
        hd = hd_output
    hd_input = np.column_stack([Bias_Input,hd])
...
for i in range(iteration):
    ...
    for k in range(batch_iteration):
        ...
        error = MLP_Learning(mini_t,mini_l,1,step,1)
...
for i in range(iteration):
    ...
    for k in range(batch_iteration):
        ...
        for j in range(batch_size):
            ...
            mini_t[j,:] = new_training[k*batch_size+index,:]
            mini_l[j,:] = tr_labels[k*batch_size+index,:]
        error = MLP_Learning(mini_t,mini_l,1,step,0)
