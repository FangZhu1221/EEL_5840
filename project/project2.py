# -*- coding: utf-8 -*-
"""
Created on Fri Dec  1 17:16:05 2017
...
J1 = np.zeros(iteration)
J2 = np.zeros(iteration)
...
for i in range(iteration):
    ...
    J2[i] = np.trace((error.T@error)/(2*batch_size))
    Out = MLP(new_validation,1,dropout)
    e = va_labels - Out
    J1[i] = np.trace((e.T@e)/(2*validation_length))
...
for i in range(iteration):
    ...
    Out = MLP(new_validation,1,dropout)
    error = va_labels - Out
    J1[i] = np.trace((error.T@error)/(2*validation_length))
