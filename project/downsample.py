# -*- coding: utf-8 -*-
"""
Created on Wed Nov 29 22:58:11 2017
...
for i in range(training_length):
    g = downsample(np.reshape(training_images[i,:,:],(row,column)),2)
    new_training[i,:,:] = np.reshape(g,(1,new_row,new_col))
for i in range(validation_length):
    g = downsample(np.reshape(validation_images[i,:,:],(row,column)),2)
...
#new_validation = downsample(validation/255,new_dimensions)
#new_training = downsample(training/255,new_dimensions)
iteration = 100
step = 0.02
