# -*- coding: utf-8 -*-
"""
Created on Mon Dec  4 21:10:29 2017
...
if new_dimensions == 784:
    ...
    if choice == 1:
        ...
        if choice == 2:
            ...
            new_testing = np.array(np.reshape(new_testing,(testing_length,new_col*new_row)))/255
                    
#%%
t = np.arange(0.4,0.9,0.05)
...
for m in range(10):
    ...
    for i in range(iteration):
        ...
        for k in range(batch_iteration):
            ...
            MLP_Learning(mini_t,mini_l,1,step,dropout,mo)
    ...
    for i in range(iteration):
        ...
        for k in range(batch_iteration):
            ...
            for j in range(batch_size):
                ...
                mini_t[j,:] = new_training[k*batch_size+index,:]
                mini_l[j,:] = tr_labels[k*batch_size+index,:]
            error2 = MLP_Learning(mini_t,mini_l,1,step,dropout,mo)
        J2[i] = np.trace((error2.T@error2)/(2*training_length))
        error1 = va_labels - MLP(new_validation,1,dropout)
        J1[i] = np.trace((error1.T@error1)/(2*validation_length))
    Out = MLP(new_validation,1,dropout)
    answer = np.argmax(Out, axis=1)
    count = 0
    for j in range(validation_length):
...
Out = MLP(new_testing,1,dropout)
answer = np.argmax(Out, axis=1)
count = 0
for j in range(testing_length):
    if answer[j] == testing_labels[j]:
        count = count + 1
accuracy = count/testing_length
print(accuracy)
matrix = confusion_matrix(testing_labels, answer)
